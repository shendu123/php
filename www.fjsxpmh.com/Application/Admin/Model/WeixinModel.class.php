        ...
        if($nurl){
            ...
            if($yn){
                ...
                if(C('Weixin.appid')&&C('Weixin.appsecret')){
                    ...
                    if($send){
                        $senddata = array($data['name'],$data['comment'],$win_url,$webroot.__ROOT__.trim(C('UPLOADS_PICPATH'),'.').$data['toppic']);
                        // 给全部用户发送图文
                        // 获取微信登陆该站小于48小时的用户
                    ...
                    return array('status' => 1, 'info' => $ynmsg.'成功！<br/>未配置微信，无法发送！', 'url' => U('Weixin/index'));
                    exit;
                }
                return array('status' => 1, 'info' => $ynmsg.'成功！'.$wresult, 'url' => U('Weixin/index'));
            }else{
                return array('status' => 0, 'info' => $ynmsg.'失败！请修改后提交，请重试！','url' =>__SELF__);
            }
        }else{
            return array('status' => 0, 'info' => '网址格式不正确！','url' =>__SELF__);
        }
    }
}
?>
