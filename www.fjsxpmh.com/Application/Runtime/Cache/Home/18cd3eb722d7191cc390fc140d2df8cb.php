...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
		...
		<meta name="description" content="<?php echo ($sinfo["description"]); ?>" />
        <title><?php echo ($sinfo["sname"]); ?>在拍商品-<?php echo ($site["SITE_INFO"]["title"]); ?></title>
...
<script type="text/javascript">
    ...
    <div class="speul_banner meeting_banner <?php if(($sinfo["status"]) == "biding"): ?>biding<?php endif; if(($sinfo["status"]) == "future"): ?>future<?php endif; if(($sinfo["status"]) == "bidend"): ?>bidend<?php endif; ?>"  <?php if(($sinfo["mbanner"]) != ""): ?>style="background:url(<?php echo ($upWholeUrl); echo ($sinfo["mbanner"]); ?>) no-repeat center 0;"<?php endif; ?> >
        <div class="speul_banner_box">
            <div class="specialInfobox">
                <div class="dowtime special_timebox">
                    <?php if(($sinfo["status"]) == "bidend"): ?><div class="endtis">本场已结束</div>
                        <?php else: ?>
                        <div class="th" id="bidTimeStatus">
                            <?php if(($sinfo["status"]) == "biding"): ?>最快结束时间<?php endif; ?>
                            <?php if(($sinfo["status"]) == "future"): ?>距离开始<?php endif; ?>
                        </div>
                        <div class="onBidtime<?php echo ($sinfo["mid"]); ?>" id="onBidtime">
                            <span class="day">-</span>
                    ...
                    <?php if(($sinfo["status"]) == "biding"): ?><script type="text/javascript">
                            $(function(){
                                meetingCountDown("cke","<?php echo ($sinfo["mid"]); ?>","<?php echo ($sinfo["endtime"]); ?>","<?php echo ($nowtime); ?>",".onBidtime<?php echo ($sinfo["mid"]); ?>",".onBidtime<?php echo ($sinfo["mid"]); ?> .day",".onBidtime<?php echo ($sinfo["mid"]); ?> .hour",".onBidtime<?php echo ($sinfo["mid"]); ?> .minute",".onBidtime<?php echo ($sinfo["mid"]); ?> .second",".onBidtime<?php echo ($sinfo["mid"]); ?> .msec");
                            });
                        </script><?php endif; ?>
                    <?php if(($sinfo["status"]) == "future"): ?><script type="text/javascript">
                            $(function(){
                                meetingCountDown("cks","<?php echo ($sinfo["mid"]); ?>","<?php echo ($sinfo["starttime"]); ?>","<?php echo ($nowtime); ?>",".onBidtime<?php echo ($sinfo["mid"]); ?>",".onBidtime<?php echo ($sinfo["mid"]); ?> .day",".onBidtime<?php echo ($sinfo["mid"]); ?> .hour",".onBidtime<?php echo ($sinfo["mid"]); ?> .minute",".onBidtime<?php echo ($sinfo["mid"]); ?> .second",".onBidtime<?php echo ($sinfo["mid"]); ?> .msec");
                            });
                        </script><?php endif; ?>
                </div>
                <!-- 结束和未结束 -->
                <?php if(($sinfo["status"]) == "bidend"): ?><div class="endtis_info">
                        <p class="f16">成交总金额</p>
                        <p class="red1 f24"><span class="f16 mr5">&yen;</span><strong><?php echo ((isset($sinfo["countprc"]) && ($sinfo["countprc"] !== ""))?($sinfo["countprc"]):"0"); ?></strong></p>
                    </div>
                <?php else: ?>
                    <div class="info_li clearfix">
                        <h2 class="red1 mb5 cuit_over"><?php echo ($sinfo["mname"]); ?></h2>
                        <p>拍品：<strong><?php echo ($sinfo["count"]); ?></strong> 件</p>
                        <p>围观：<strong><?php echo ($sinfo["clcount"]); ?></strong>次</p>
                        <p>保证金：<strong>
                            <?php if(($sinfo["meeting_pledge_type"]) == "0"): echo ((isset($sinfo["spledge"]) && ($sinfo["spledge"] !== ""))?($sinfo["spledge"]):"0"); ?>元<?php endif; ?>
                            <?php if(($sinfo["meeting_pledge_type"]) == "1"): ?>分别收取<?php endif; ?></strong>
                        </p>
                        <p>出价：<strong><?php echo ((isset($sinfo["scount"]) && ($sinfo["scount"] !== ""))?($sinfo["scount"]):0); ?></strong>次</p>
                    </div><?php endif; ?>
                <div class="info_de">
                    <?php echo ($sinfo["description"]); ?>
                </div>
            </div>
        </div>
    </div>
    <div class="sort-bar-wrap">
        <div class="sort-bar">
            <div class="sort-orders">
                <a href="<?php echo U('Meeting/meetul',array('mid'=>$sinfo['mid']));?>" <?php if(($sinfo["pd"]) == ""): ?>class="current"<?php endif; ?> >默认</a>
                
                <?php if(($sinfo["status"]) != "future"): ?><a href="<?php echo U('Meeting/meetul',array('mid'=>$sinfo['mid'],'po'=>$sinfo['po']));?>" class="descending <?php if(($sinfo["pd"]) == "po"): ?>current<?php endif; ?>">出价次数 <span class="icontxt_pai"><?php if(($sinfo["po"]) == "0"): ?>▼<?php endif; if(($sinfo["po"]) == "1"): ?>▲<?php endif; ?></span></a><?php endif; ?>

                <a href="<?php echo U('Meeting/meetul',array('mid'=>$sinfo['mid'],'bc'=>$sinfo['bc']));?>" class="descending <?php if(($sinfo["pd"]) == "bc"): ?>current<?php endif; ?> ">价格<span class="icontxt_pai"><?php if(($sinfo["bc"]) == "0"): ?>▼<?php endif; if(($sinfo["bc"]) == "1"): ?>▲<?php endif; ?></span>
                </a>                              
            </div>
        </div>
    </div>
    <div  class="container speul_list">
        <div class="auctionbox clearfix">
            <?php if(is_array($bidArr)): foreach($bidArr as $key=>$bav): if(empty($bav["bid_list"])): ?><div class="datanone">很抱歉该专场下暂无拍品</div> 
                <?php else: ?>
...
<script type="text/javascript">
var ajaxGetMeetingTime = "<?php echo U('Meeting/ajaxmeetinggettime');?>";
var ajaxCheckMeetingSucc = "<?php echo U('Meeting/checkmeetingsucc');?>";
var endDowntimer = [];
// 结束倒计时
