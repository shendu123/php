...
class Goods extends Model
{
   ...
   function make_dir($path){ 
    if(!file_exists($path)){//不存在则建立 
        $mk=@mkdir($path,0777,true); //第三个参数是“true”表示能创建多级目录
        @chmod($path,0777); 
    } 
    return true; 
} 
...
function save_pic($url,$savepath=''){ echo $url;
    //处理地址 
    $url=trim($url); 
    ...
    function get_pic($cont,$path){ 
        ...
        $pattern_src = '/[.*]?src=[\'\"]?([^\'\"]*)!142[\'\"]?.*?/'; 
        $num = preg_match_all($pattern_src, $cont, $match_src); //dump($match_src);exit;
        $pic_arr = $match_src[1]; //获得图片数组 
        //dump($cont);
        foreach ($pic_arr as $pic_item) { //循环取出每幅图的地址 
            $file = $this->save_pic($pic_item,$path); //下载并保存图片 
