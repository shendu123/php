angular.module('pay.companyCash.index', [

]).controller('controller.pay.companyCash.index', ["$http", "$scope", "$state", "$rootScope", "$stateParams", "utils", '$modal', "API_WD_DOMAIN", "pay.companyCash.index", function($http, $scope, $state, $rootScope, $stateParams, utils, $modal, HOST, memberService) {
            ...
            controller: ["$scope", "$rootScope", "$modalInstance", "$state", "utils", function($scope, $rootScope, $modalInstance, $state, utils) {
                ...
                if(tpl == 'tpl-checkPass.html'){
                    $scope.checkPassSubmuit = function(formData) { 
                        ...
                        url = HOST + '/finance/withdraw/businessCheck';
