...
class Member extends base
{
    ...
    public function adjustMoney(){
        $waModel=new \app\finance\model\WalletAction();
        $data=request()->param();
        if(empty($data['money']) && empty($data['point']) && empty($data['wallet_freeze'])){
            $this->_error('数据不能为空', 400);
        }
        if((isset($data['money']) && !is_numeric($data['money']))||(isset($data['point']) && !is_numeric($data['point']))) {
            $this->_error('格式错误', 400);
        }
		$wallet = [];						
        if(isset($data['money'])){
            $data['money']=$data['opMoney']>0?$data['money']:'-'.$data['money'];
			$wallet['wallet_available_price'] = ['exp' , 'wallet_available_price' . ($data['money'] > 0 ? '+'.$data['money'] : $data['money'])];
        }
        if(isset($data['point'])){
            $data['point']=$data['opPoint']>0?$data['point']:'-'.$data['point'];
			$wallet['wallet_integral'] = ['exp' , 'wallet_integral' . ($data['point'] > 0 ? '+'.$data['point'] : $data['point'])];
        }
        if(isset($data['wallet_freeze'])){
            $data['wallet_freeze'] = $data['opWallet_freeze']>0?$data['wallet_freeze']:'-'.$data['wallet_freeze'];
			$wallet['wallet_freeze_price'] = ['exp' , 'wallet_freeze_price' . ($data['wallet_freeze'] > 0 ? '+'.$data['wallet_freeze'] : $data['wallet_freeze'])];
        }      
		...
		Db::startTrans();
		try{
			if(!Db::table('op_finance.opf_wallet_action')->insert($data)){
				throw new \Exception('钱包记录添加失败');
			}
			if(!$wallInfo = Db::table('op_finance.opf_wallet')->where([ 'user_id' => $data['uid'] ])->find()){
				throw new \Exception('没有生成钱包');
			}
			if(!Db::table('op_finance.opf_wallet')->where([ 'user_id' => $data['uid'] ])->update($wallet)){
