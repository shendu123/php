...
class User extends NoAuth {
    ...
    public function index() {        
		...
		$reg_date = request()->param('start_date');
        $where=[];
        if($keyword){
            $where['mobile|account']=['like',"%".$keyword."%"];
        }
		if($reg_date){
			$where['reg_date'] = ['egt' , strtotime($reg_date)];
		}
        $users = model('Member')->where($where)->field('uid,email,business_id,account,nickname,reg_date,truename,login_time,mobile,avatar,sex')->paginate($this->request->param('s'))->toArray();
        foreach($users['data'] as $index => &$user) {
            $user['login_time']= $user['login_time'] ? date('Y-m-d H:i:s',$user['login_time']) : '--';
			$user['reg_date'] = $user['reg_date'] ? date('Y-m-d H:i:s',$user['reg_date']) : '--';
    ...
    public function add(){
        ...
        if(request()->isPost()){
            ...
            try{
                ...
                Db::table('op_finance.opf_wallet')->insert($waData);               
                Db::commit();
                return ['msg'=> '添加成功'];                 
            }catch(\Exception $e){
                Db::rollback();
