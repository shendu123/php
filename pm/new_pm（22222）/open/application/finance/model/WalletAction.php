...
class WalletAction extends InitModel {
    protected $table = 'op_finance.opf_wallet_action';
    ...
    public function getListByUid($page,$where){
        $res=Db::table($this->table)->where($where)->order('createtime desc')->limit(($page['page']-1)*$page['pageSize'],$page['pageSize'])->select();
