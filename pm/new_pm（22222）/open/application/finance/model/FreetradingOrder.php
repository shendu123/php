 ...
 * @author ljx
 */
class FreetradingOrder extends Model
{

    protected $table = 'opf_freetrading_order';

    public $_tableFields = array(
        'order_id' => 'int', // 自由买卖订单id
    ...
    public function insert($requestData = array(), $operateData = array()){
        ...
        $result = Db::name('freetrading_order')->insert($fields);
        $id = Db::name('freetrading_order')->getLastInsID();
        if($result === false){
            return false;
        }else{
            return $id;
    ...
    public function edit($requestData = array(), $operateData = array())
    {
        ...
        // case 详情表  goods_id + detail_goods_attr_value TODO detail_goods_attr_value
        if(isset($requestData['goods_id']) && !empty($requestData['goods_id'])){
            $detailModel = new \app\finance\model\AuctionOrderDetail();
            $wdata_detail = array(
                'goods_id' => $requestData['goods_id'],
                //'detail_goods_attr_value' => $requestData['detail_goods_attr_value'],
                'order_id' => $fields['order_id']
            );
    ...
    public function editBy($wdata = array(), $requestData = array(), $operateData = array())
    {
        ...
        return Db::table('opf_freetrading_order')->where($wdata)->update($fields);
    }
    ...
    public function delete($wdata = array(), $operateData = array()){
        ...
        // case service_order
        $result_service = Db::table('opf_freetrading_order')->where($wdata)->delete();
    ...
    public function getBy($wdata = array(), $requestData = array()){
        ...
        return Db::table('opf_freetrading_order')->where($wdata)->field($sql_fields)->find();
    }
    ...
    public function getList($wdata = array(), $exchange = true)
    {
        ...
        $list = Db::table('opf_freetrading_order')->alias('freetrading')
            ->where($whereCond)
            ->field('freetrading.*')
            ->order('freetrading.order_id desc')
            ->limit(($wdata['page'] - 1) * $wdata['pageSize'] . ',' . $wdata['pageSize'])
            ->select();
        
        $count = Db::table('opf_freetrading_order')->alias('freetrading')
            ->where($whereCond)
            ->count();
    ...
    public function getRow($requestData = array(), $operateData = array(), $exchange = true){
        ...
        $list = Db::table('opf_freetrading_order')
        ->alias('o')
        ->join('opf_freetrading_order_detail od', 'od.order_id = o.order_id', 'INNER')
        ->where($wdata)
        ->select();

        if(!empty($list) && $exchange === true){
            $order = $orderDetail = array();
            $orderFields = array_keys($this->_tableFields);
            //$detailModel = new \app\finance\model\FreetradingOrderDetail();
            //$orderDetailFields = array_keys($detailModel->_tableFields);
            foreach($list as $key => $val){
                foreach($val as $key_in => $val_in){
                    if(in_array($key_in, $orderFields)){
                        $order[$key_in] = $val_in;
                    }else{
                        // 那一定是属于详情的
                        $tmp[$key_in] = $val_in;
     ...
     * @author zsq
     */
    public function getOrderStatus($orderId, $stuffType)
    {
        $orderInfo = Db::table('opf_freetrading_order')
                        ->where("order_code = '{$orderId}'")
                        ->field('order_id,business_id,order_status')
                        ->find();                
        if(empty($orderInfo)){
            return ['status'=>'fail', 'data'=>[]];
        }
        $id = $orderInfo['order_id']; 
        $orderDetail = Db::table('opf_freetrading_order_detail')
                        ->where("order_id = '{$id}'")
                        ->field('goods_id,detail_stuff_id,detail_num,detail_goods_price,detail_goods_attr_value,detail_goods_attr_str')
                        ->select();
        $orderInfo['order_detail'] = $orderDetail;                  
