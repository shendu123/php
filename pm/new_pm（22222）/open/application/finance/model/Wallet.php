...
class Wallet extends Model
{
    ...
    public function raise($requestData = array(), $operateData = array())
    {
        ...
        if(!empty($requestData['wallet_id'])){
            $condition['wallet_id'] = $requestData['wallet_id'];   
        }
        return Db::table('opf_wallet')->where($condition)->update([
     ...
     * @param $requestData['amount'] 金额
     */
    public function freezePrice($requestData = array(), $operateData = array())
    {
        if(!empty($requestData['user_id'])){
            $condition['user_id'] = $requestData['user_id'];
        }
