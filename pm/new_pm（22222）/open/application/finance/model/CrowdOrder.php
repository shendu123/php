 ...
 * @author ljx
 */
class CrowdOrder extends Model
{

    protected $table = 'opf_crowd_order';
    ...
    public function delete($wdata = array(), $operateData = array()){
        ...
        // case service_order
        $result_service = Db::table('opf_crowd_order')->where($wdata)->delete();
    ...
    public function getBy($wdata = array(), $requestData = array()){
        ...
        return Db::table('opf_crowd_order')->where($wdata)->field($sql_fields)->find();
    }
    ...
    public function getList($wdata = array(), $exchange = true)
    {
        ...
        $list = Db::table('opf_crowd_order')->alias('crowd')
            ->where($whereCond)
            ->field('crowd.*')
            ->order('crowd.order_id desc')
            ->limit(($wdata['page'] - 1) * $wdata['pageSize'] . ',' . $wdata['pageSize'])
            ->select();
        
        $count = Db::table('opf_crowd_order')->alias('crowd')
            ->where($whereCond)
            ->count();
    ...
    public function getRow($requestData = array(), $operateData = array(), $exchange = true)
    {
        ...
        $row = Db::table('opf_crowd_order')->alias('o')
            ->join('opf_crowd_order_detail od', 'od.order_id = o.order_id', 'INNER')
            ->where($wdata)
            ->find();
     ...
     * @author zsq
     */
    public function getOrderStatus($orderCode, $stuffType)
    {
        $orderInfo = Db::table('opf_crowd_order')
        ...
        $orderDetail = Db::table('opf_crowd_order_detail')
                        ->where("order_id = '{$id}'")
                        ->field('detail_stuff_id,detail_num,detail_goods_price')
                        ->find();
        $result = array_merge($orderInfo, $orderDetail);
