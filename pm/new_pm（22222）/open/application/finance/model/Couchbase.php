...
class Couchbase
{
    ...
    public function updateServiceOrderFields($wdata = array(), $fields = array(), $operateData = array())
    {
        ...
        if (isset($fields['express_id']) && isset($orderDetail['express_id'])) {
            $sql_fields .= ", express_id = {$fields['express_id']} ";
        }
        if (isset($fields['order_express_info']) && isset($orderDetail['order_express_info'])) {
            $sql_fields .= ", order_express_info = '{$fields['order_express_info']}' ";
        }
        if (isset($fields['order_deliverid']) && isset($orderDetail['order_deliverid'])) {
            $sql_fields .= ", order_deliverid = {$fields['order_deliverid']} ";
        }
        if (isset($fields['order_deliver_time']) && isset($orderDetail['order_deliver_time'])) {
            $sql_fields .= ", order_deliver_time = {$fields['order_deliver_time']} ";
        }
    ...
    public function getList($wdata = array(), $operateData = array(), $exchange = true)
    {
        ...
        if(isset($wdata['isdelivery']) && empty($wdata['ispay']) && $wdata['isdelivery'] !== NULL && $wdata['ispay']===NULL){
            $sql_where .= " AND order_status = {$wdata['isdelivery']} ";
        }
        if(empty($wdata['isdelivery']) && isset($wdata['ispay']) && $wdata['isdelivery']===NULL && $wdata['ispay'] !== NULL){
            $sql_where .= " AND order_status = {$wdata['ispay']} ";
        }
        if(isset($wdata['isdelivery']) && isset($wdata['ispay']) && $wdata['isdelivery'] !== NULL && $wdata['ispay']!==NULL){
            $sql_where .= " AND ( order_status = {$wdata['ispay']} or order_status = {$wdata['isdelivery']} )  ";
        }
     ...
     * @author zsq
     */
    public function getUserListBy($where = [], $userId){
        // TODO 稳定后 要指定字段 或者考虑接口专用
        $sql_select = "SELECT * ";
        $sql_from = " FROM finance_order ";
        $sql_where = " WHERE 1=1 ";
        ...
        // 联合订单号严格搜索
        $sql_where .= " and  order_buyer.user_id ={$userId}";
        $sql_where .= " and (stuff_type = '1' or stuff_type='2' or stuff_type = '3') ";
        if(isset($where['order_status'])){
            $sql_where .= " and order_status = {$where['order_status']}";
        }
        // 获取数据
        $sql_all = $sql_select . $sql_from . $sql_where;
