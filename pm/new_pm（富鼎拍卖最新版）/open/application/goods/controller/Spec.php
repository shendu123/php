...
class Spec extends Base {
	...
	public function add() {
		...
		$data['name'] = $this->request->post('spec_name');
		...
		return $rs ? ['error'=>0, 'spec_id'=>$rs] : $this->_error('商品规格添加失败', 500);
	}
	...
	public function edit() {
		...
		$data['name'] = $this->request->post('spec_name');
		$data['sort'] = (int)$this->request->post('sort');
		if ($id == 0 || $data['cat_id'] == 0 || empty($data['spec_name'])) {
			$this->_error('参数错误', 400);
		}
		$data['oprid'] = $this->_uid;
		$data['uptime'] = time();
		...
		return $rs ? ['error'=>0, 'msg'=>'商品规格修改成功'] : $this->_error('商品规格修改失败', 500);
	}
