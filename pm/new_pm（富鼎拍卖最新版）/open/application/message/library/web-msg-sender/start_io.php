...
$sender_io->on('connection', function ($socket) {
    ...
    $socket->on('push_msg_by_type', function ($push_msg_type) use ($socket) {
        // 触发所有客户端定义的chat message from server事件
        if($push_msg_type == 1){
            $socket->emit('push_msg_response', "response for push_msg_type: " . $push_msg_type);
        }else{
            $socket->emit('push_msg_response', "common response");            
        }
    });
