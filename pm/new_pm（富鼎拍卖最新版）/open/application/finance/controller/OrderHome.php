...
use think\Db;
use app\finance\controller\Payquery;
    ...
    public function isPaid($stuff_type_param = 0, $order_id_param = 0)
    {
        ...
        if ($row['order_status'] != 2) {
            ...
            switch($row['order_pay_type']){
                //微信微信支付
                case 1:
                    $result = $payQuery->wechat($row['order_code']);
                    if(isset($result['trade_state']) && $result['trade_state'] == 'SUCCESS'){
                        $payReturn->paid($row['order_id'], $stuff_type, $row['order_code']);           
                        $isPaid = true;
                    }
                    if($stuff_type == 4){
                        $result = $payQuery->wechat($row['order_sn']);
                        if(isset($result['trade_state']) && $result['trade_state'] == 'SUCCESS'){
                            $payReturn->paid($row['order_id'], $stuff_type, $row['order_code']);
                            $isPaid = true; 
                        }
