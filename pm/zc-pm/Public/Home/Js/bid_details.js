...
$(function(){
    ...
    // 机器人ajax自动提交
    //postbid(acpid,'rob_'+ws_rob_uid,'sd',$('#bidprice').val());
    
    function robotExcute(){
//        $.ajaxSetup({   
//            async : false  
//        }); 
        $.post(bidUrl,{'pid':acpid,'uid':ws_rob_uid,'bidType':'sd','bidPric':'','agr':'robot'},function(data){//console.log(data.msg);
            if(data.status==1){
               $('#nowprice').text(data.thisS.nowprice);
               if($('#bidprice')){
                   $('#bidprice').val(data.thisS.bidprice);
               }
               $('tr.lingxian').removeClass('lingxian');
               if(data.thisS.recordList){
                    var bid_item = '';
                    var bid_itemall = '';
                    $.each(data.thisS.recordList,function(drk,drv){
...
// 刷新用户列表框
    function flush_client_list(){
        ...
        var url="http://"+http_host+'/index.php/Auction/onlineUserList?pid='+acpid;
        $.get(url,function(data){
            if(data.status==1){
                for(var p in data.data){
                     userlist_window.append('<li id="'+p+'">'+data.data[p]+'</li>');
                     client_list_slelect.append('<option value="'+p+'">'+data.data[p]+'</option>');
                }                
            }
        });
