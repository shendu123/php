...
class WechatAuth {
    ...
    private function _initAccessToken() {
        if(S('S_accessToken')){
            $this->accessToken = S('S_accessToken');
        } else {
            $this->_resetAccessToken();
        }
    }

    private function _resetAccessToken() {
        $S_accessToken = $this->getAccessToken();
     ...
     */
    public function getUserInfo($openid, $lang = 'zh_CN', $retry = true){
        $info = self::http("{$this->oauthApiURL}/userinfo", array(
        ...
        if($user['errcode'] && $retry) {
            $this->_resetAccessToken();
            $user = $this->getUserInfo($openid, $lang, false);
        }
    ...
    public function mediaUpload($filename, $type){
        ...
        return $this->api('media/upload', array(
            'type'  => $type,
            'media' => "@{$filename}"
        ), 'POST', '', false);
    }
    ...
    public function mediaGet($media_id){
        return "{$this->apiURL}/media/get?" . http_build_query(array('access_token' => $this->accessToken, 'media_id' => $media_id));
    }
    ...
    public function sendNewsOnce(){
        $news   = func_get_args();
        $openid = array_shift($news);
        return $this->messageCustomSend($openid, array($news), self::MSG_TYPE_NEWS);
    }
    ...
    public function groupsCreate($name){
        return $this->api('groups/create', array('group' => array('name' => $name)));
    }
    ...
    public function groupsGetid($openid) {
        return $this->api('groups/getid', array('openid' => $openid));
    }
    ...
    public function groupsUpdate($id, $name){
        return $this->api('groups/update', array('id' => $id, 'name' => $name));
    }
    ...
    public function groupsMemberUpdate($openid, $to_groupid){
        return $this->api('groups/member/update', array('openid' => $openid, 'to_groupid' => $to_groupid));
    }
    ...
    public function userInfoUpdateremark($openid, $remark){
        return $this->api('user/info/updateremark', array('openid' => $openid, 'remark' => $remark));
    }
    ...
    public function userInfo($openid, $lang = 'zh_CN'){
        return $this->api('user/info', '', 'GET', array('openid' => $openid, 'lang' => $lang));
    }
    ...
    public function userGet($next_openid = ''){
        return $this->api('user/get', '', 'GET', array('next_openid' => $next_openid));
    }
    ...
    public function menuCreate($button){
        return $this->api('menu/create', array('button' => $button));
    }
    ...
    public function shorturl($long_url){
        return $this->api('shorturl', array('action' => 'long2short', 'long_url' => $long_url));
    }
     ...
     */
    protected function api($name, $data = '', $method = 'POST', $param = '', $json = true, $retry = true){
        $args = func_get_args(); $args[5] = false;
        ...
        $data = json_decode(self::http($url, $params, $data, $method), true);

        if($data['errcode'] && $retry) {
            $this->_resetAccessToken();
            $data = call_user_func_array(array($this, 'api'), $args);
        }
