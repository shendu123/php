...
class AlipayNotify{
	...
	function verifyNotify(){
		if(empty($_POST)) {//判断POST来的数组是否为空
			...
			var_dump(__DIR__ . '/alipay_md5.function.php');exit();
			//生成签名结果
	...
	function verifyReturn(){
		if(empty($_GET)) {//判断POST来的数组是否为空
			...
			$isSign = $this->getSignVeryfy($_GET, $_GET["sign"]);//生成签名结果
			$responseTxt = 'true';
			if (! empty($_GET["notify_id"])) {
				$this->logali->info($_GET["notify_id"]);
				$responseTxt = $this->getResponse($_GET["notify_id"]);//验证是否是支付宝发来的消息
			}

			if (!$isSign) {
				$this->logali->info("---签名失败---");
			}
			
			if(!$responseTxt){
				$this->logali->info("---非支付宝---");
			}
