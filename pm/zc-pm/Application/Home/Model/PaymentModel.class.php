...
class PaymentModel extends ViewModel {
    ...
    public function yuepaySend($ainfo, $money, $order_no, $uid){
        $from = $this->order_from_by($order_no);
        if($from == 'pm') {
            $member = M('Member');
            ...
            // 变动方式changetype 竞拍冻结bid_freeze 竞拍解冻bid_unfreeze 后台充值admin_deposit 管理员扣除 admin_deduct 支付充值pay_deposit 支付扣除pay_deduct  提现extract  pay_crowd申购支付
            $pledge_data = array(
                'order_no'=>$order_no,
                'uid'=>$uid,
            ...
            // $staz = $member->where(array('uid'=>$uid))->setInc('point',$money/10);
            ...
            $member = M('Member');
            $mod = '申购商品：“<a href="'.U('Home/Crowd/items',array('cid'=>$ainfo['crowd_id'])).'">'.$ainfo['title'].'</a>”';
            // 变动方式changetype 竞拍冻结bid_freeze 竞拍解冻bid_unfreeze 后台充值admin_deposit 管理员扣除 admin_deduct 支付充值pay_deposit 支付扣除pay_deduct  提现extract  pay_crowd申购支付
            $pledge_data = array(
                'order_no'=>$order_no,
                'from' => 'zc',
                'uid'=>$uid,
                'changetype'=>'pay_crowd',
                'time'=>time(),
                'annotation'=>'支付'.$mod.'，支付成功！',
                'expend'=>$money,
            );
            ...
            if(M('member_pledge_bill')->add($pledge_data)){
                ...
                $wei_yupay['msg']=array(
                    "url"=>U('Member/pledge','','html',true),
                    "first"=>"您好，".'使用【账户余额】支付申购订单成功！',
                    "remark"=>'查看账户记录>>',
                    "keyword"=>array('余额账户','支付申购订单扣除余额','商品订单:'.$order_no,'-'.$money.'元',$usable.'元')
                );
                // 账户类型，操作类型、操作内容、变动额度、账户余额
                // 站内信提醒内容
                $web_yupay = array(
                    'title'=>'支付订单',
                    'content'=>'支付'.$mod.'扣除余额'.$money.'元'
                );
                // 短信提醒内容
                if(mb_strlen($ainfo['title'],'utf-8')>10){
                    $newname = mb_substr($ainfo['title'],0,10,'utf-8').'...';
                }else{
                    $newname = $ainfo['title'];
                }
                $note_yupay = '使用余额支付申购商品“'.$newname.'”订单号'.$order_no.'扣除【'.$money.'元】，账户可用余额【'.$usable.'元】您可以登陆平台查看账户记录。';
                // 邮箱提醒内容
                $mail_yupay['title'] = '支付订单成功';
                $mail_yupay['msg'] = '您好：<br/><p>'.$pledge_data['annotation'].'</p><p>您可以<a target="_blank" href="'.U('Home/Member/pledge','','html',true).'">查看账户记录</a></p>';
    ...
    public function order_from_by($order_no) {
        $from = 'pm';
        $pre = substr($order_no, 0, 3);
        switch ($pre) {
            case 'CRD':
                $from = 'zc';
                break;
