 ...
 * +----------------------------------------------------------
 */
function getFiltrateHtml($cid, $gt) {
    if (!S('FiltrateHtml_' . $cid . '_' . 'FiltrateHtml_' . $cid . '_' . implode('_', $gt))) {
        $filtrate = M('Goods_filtrate');
        $cate = M('Goods_category');
        $cLinkF = M('Goods_category_filtrate');
        $cat = new \Org\Util\Category('Goods_filtrate', array('fid', 'pid', 'name'));
        $fisgt = implode('_%', explode('_', $gt[4]));
        $filtArr = explode('_', $gt[4]);
        ...
        foreach ($catPath as $lk => $lv) {
            ...
            if ($filtMap) {
                // 获取不限条件fid
                $topChildMap = $filtrate->where('pid= 0')->select();
                $top_layer = $filtrate->where('pid = 0')->getField('fid', true);
                // 顶级
                ...
                foreach ($filtMap as $fk => $fv) {
                    ...
                    if (empty($gt[4]) || in_array($fv['fid'], $filtArr)) {
                        $filtHtml .= ' current';
                    }
                    // 不限条件地址生成
                    foreach ($makeArr as $key => $fiv) {

                        if ($filtrate->where('fid=' . $fiv)->getField('pid') == 0) {
                            $topNew_gt = '';
                            foreach ($cat->getList(NULL, $fiv, NULL) as $onek => $onev) {
                                $topNew_gt = str_replace('_%' . $onev['fid'] . '_%', '_%' . $fiv . '_%', '_%' . $fisgt . '_%');
                            }
                            $topSunLayer = $filtrate->where('pid = 0')->getField('fid', true);
                            foreach ($topSunLayer as $tsk => $tsv) {
                                if ($tsv == $fiv) {
                                    $topNew_gt = '_%' . $fisgt . '_%';
                                    foreach ($cat->getList(NULL, $fiv, NULL) as $tglk => $tgl) {
                                        if ($tgl['fid'] == $fiv) {
                            ...
                            $topNew_gt = '';
                            $tpfid = $filtrate->where('fid=' . $fv['fid'])->getField('pid');
                            $topSunLayer = $filtrate->where('pid=' . $fv['fid'])->getField('fid', true);

                            foreach ($topSunLayer as $tsk => $tsv) {
                                if ($tsv == $fiv) {
                                    $topNew_gt = str_replace('_%' . $fiv . '_%', '_%' . $fv['fid'] . '_%', '_%' . $fisgt . '_%');
                                    $fivlist = $cat->getList(NULL, $fiv, NULL);
                                    if (is_array($fivlist)) {
                    ...
                    $fixPath = $cat->getPath($fv['fid']); //获取该分类路径
                    // foreach ($fixPath as $fpk => $fpv) {
                    //     $fixPath
                    // }
                    // 获取该条件下一级条件fid集合
                    $same_layer = $filtrate->where('pid=' . $fv['fid'])->getField('fid', true);
                    $fixsunArr = array();
                    $childLi = '';
                    foreach ($childMap as $ck => $cv) {
                        ...
                        foreach ($same_layer as $sly => $slv) {
                            foreach ($filtArr as $onfak => $onfav) {
                                $searchArr = $fixsunArr;
                                ...
                                if ($slv == $onfav) {
                                    ...
                                    foreach ($cat->getList(NULL, $onfav, NULL) as $glk => $gl) {
                        ...
                        // 一级筛选条件地址生成_end
                        $ahref = U(ACTION_NAME, array('gt' => $gt[0] . '-' . $gt[1] . '-' . $gt[2] . '-' . $gt[3] . '-' . substr($new_gt, 1, strlen($new_gt) - 2) . '-' . $gt[5] . '-' . $gt[6]));
                        ...
                        $childLi .= getChildHtml($cv['fid'], $gt, $display, $filtArr); //获取子类
                    }
                    $filtHtml .= '</ul>';
                    $filtHtml .= $childLi;
                    $childLi = '';
        ...
        S('FiltrateHtml_' . $cid . '_' . implode('_', $gt), $filtHtml);
    } else {
        $filtHtml = S('FiltrateHtml_' . $cid . '_' . implode('_', $gt));
    }
...
// 通过fid生成子级筛选条件html
function getChildHtml($fid, $gt, $display, $filtArr) {
    if (countChild($fid) != 0) {
        $cat = new \Org\Util\Category('Goods_filtrate', array('fid', 'pid', 'name'));
        $filtrate = M('Goods_filtrate');
        $childArr = $filtrate->where('pid=' . $fid)->order('sort desc')->select();
        // 同一级条件fid
        $same_layer = $filtrate->where('pid=' . $fid)->getField('fid', true);
        ...
        foreach ($childArr as $ck => $cv) {
            ...
            foreach ($same_layer as $sly => $slv) {
                foreach ($filtArr as $fak => $fav) {
                    ...
                    if ($slv == $fav) {
                        ...
                        $new_gt = str_replace('_%' . $searchStr . '_%', '_%' . $replaceStr . '_%', '_%' . $fisgt . '_%');
                        // 删除子级fid
                        foreach ($cat->getList(NULL, $fav, NULL) as $glk => $gl) {
                            $new_gt = str_replace('_%' . $gl['fid'] . '_%', '_%', $new_gt);
                        }
                        break;
                    } else {
                        ...
                        $new_gt = str_replace('_%' . $searchStr . '_%', '_%' . $replaceStr . '_%', '_%' . $fisgt . '_%');
                        // 删除同级其他fid
                        foreach ($filtrate->where('pid=' . $fid)->getField('fid', true) as $glk => $gl) {
                            if ($gl != $cv['fid']) {
                                $new_gt = str_replace('_%' . $gl . '_%', '_%', $new_gt);
                            }
                            $catgetList = $cat->getList(NULL, $gl, NULL);
                            if ($catgetList) {
                                foreach ($cat->getList(NULL, $gl, NULL) as $glk => $gl) {
                                    $new_gt = str_replace('_%' . $gl['fid'] . '_%', '_%', $new_gt);
                                }
                            }
                        }
            ...
            $childStr .= '</a></li>';
            $childSun = getChildHtml($cv['fid'], $gt, $displayCh, $filtArr);
        }
        $childStr .= '</ul>';
        $childStr .= $childSun;
        $childStr .= '</div>';
 ...
 */
/**
 * [bidChannel 拍品列表需要公用的提出来了]
 * @param  [type] $channel [传进来的频道或分类cid]
 * @return [type]          [$gt0：频道pid，$cat_inCid：频道下商品的条件，$bid_inCid:分类下商品的条件]
 */
function bidChannel($channel) {
    $cate = M('Goods_category');
    $cat = new \Org\Util\Category('Goods_category', array('cid', 'pid', 'name'));
    $chan_yn = $cate->where(array('cid' => $channel))->getField('pid');
    //-------根据获取频道或分类cid显示分类
    if ($chan_yn == 0) {
        $catArr = $cat->getList(NULL, $channel, NULL); //获取频道下分类结构
        $gt0 = $channel;
    } else {
        $path = $cat->getPath($channel);
        $gt0 = $path[0]['cid'];
        $catArr = $cat->getList(NULL, $path[0]['cid'], NULL); //获取频道下分类结构

    }
    foreach ($catArr as $cak => $cav) {
        $catArr[$cak] = $cav['cid'];
    }
    $catArr[] = $channel;  //如果是频道把频道cid也加进去
    $bidArr = $cat->getList(NULL, $channel, NULL); //拍卖条件
    foreach ($bidArr as $bik => $civ) {
        $bidArr[$bik] = $civ['cid'];
    }
    $bidArr[] = $channel;
    $cat_inCid = array('cid' => array('in', $catArr));//该频道的拍品
    $bid_inCid = array('cid' => array('in', $bidArr));//该分类的拍品

    return array('gt0' => $gt0, 'cat_inCid' => $cat_inCid, 'bid_inCid' => $bid_inCid);
}
/**
 * 正在拍卖结束时间段查询条件
 * @param  [str] $gt [条件]
...
function bidRegion($ctidStr){
    $reg_where=array();
    $regArr=explode('_', $ctidStr);
    if($regArr[0]>1){
    $reg_where['province']=$regArr[0];
        if($regArr[1]>1){
          $reg_where['city']=$regArr[1];
          if($regArr[2]>1){
            $reg_where['area']=$regArr[2];
          }
        }
    }
    return $reg_where;
...
// 通过分类cid生成扩展字段html
function getExtendsCon($cid,$gid,$who='web'){
    ...
    foreach ($catPath as $lk => $lv) {
        ...
        if(is_array($eMap)){
            foreach ($eMap as $ek => $ev) {
                ...
                $eHtmlUl .='<li class="ext" eid="ext'.$ev['eid'].'">'.$ev['name'].'</li>';
                $eArrUl[$ev['eid']] = $ev['name'];
                ...
                $eHtmlDiv .='<div class="hide ext tab_con editbox" eid="ext'.$ev['eid'].'">'.htmlspecialchars_decode(stripslashes($fieldVal)).'</div>';
                $eArrDiv[$ev['eid']] = htmlspecialchars_decode(stripslashes($fieldVal));
            }
        }
        
    }
    if($who=='web'){
        return array('eUrlHtml'=>$eHtmlUl,'eDivHtml'=>$eHtmlDiv,'region'=>$region);
    }else{
        return array('eArrUl'=>$eArrUl,'eArrHtml'=>$eArrDiv);
    }
}
...
function channelAuction($limi){
    // 拍卖频道
    $channel = M('goods_category')->where('pid=0')->order('sort desc')->select(); //读取频道
    $auction = D('Auction');
    ...
    // 循环获取频道下分类并读取分类下的热拍拍品
    foreach ($channel as $ck => $cv) {
        $cateArr = array();
         $cate=$cat->getList(NULL, $cv['cid'],'sort desc');
        foreach ($cate as $ctk => $ctv) {
            $cateArr[] = $ctv['cid'];
        }
        $cateArr[]=$cv['cid'];
        $channel[$ck]['bid'][0]['elistA'] = $auction->where($where)->where(array('cid'=>array('in',$cateArr)))->order('bidcount desc,endtime')->limit($limi)->select();
        $channel[$ck]['bid'][0]['abc'] = 0;
    }
    ...
    if ($info['nowprice'] >= $info['price'] && $info['uid'] != 0) {
        ...
        if ($auction->save($aData)) {
            ...
            $oData = array(
                ...
                'time' => time(),
                'status' => 0,
                'remark' => '竞拍成功，请在订单有效期内进行支付！',
                'business_id' => M('Auction')->where(array('pid' => $info['pid']))->field('business_id')->find()['business_id'],
            );
            // 设置订单支付过期时间
            ...
            if ($yn == 0) {
                ...
                if ($order->add($oData)) {
                    // 退还保证金
                    return_pledge($info['pid'], $info['sid'], $info['uid'], $info['endstatus']);
                    if ($info['sid'] != 0) {
        ...
        if ($info['uid']) {
            ...
            $info['endstatus'] = 2;
            // 返还买家保证金
            return_pledge($info['pid'], $info['sid'], 0, $info['endstatus']);
        } else {
            // 无人出价流拍
            $aData['endstatus'] = 3;
            $info['endstatus'] = 3;
...
function verifyME($tp,$how,$adr,$uid){
    if($tp=='email'){
        $vid = M('member')->where(array('email'=>$adr))->getField('uid');
        if($how=='findpwd'){
            if($vid){
                return array('status'=>1);
            }else{
                return array('status' => 0, 'info' => "该邮箱未占用，或未进行过认证！");
            }
        }
        if($how=='register'){
            if(!$vid){
        ...
        if($how=='verify'){
            if(!$vid){
                ...
                $vuid = M('member')->where(array('email'=>$adr,'verify_email'=>1))->getField('uid');
                if(!$vuid){
                    return array('status'=>1);
                }else{
                    return array('status' => 0, 'info' => "您的邮箱已被占用请更换！");
    ...
    if($tp=='mobile'){
        $vid = M('member')->where(array('mobile'=>$adr))->getField('uid');
        if($how=='findpwd'){
            if($vid){
                return array('status'=>1);
            }else{
                return array('status' => 0, 'info' => "该手机号未占用，或未进行过认证！");
            }
        }
        if($how=='register'){
            if(!$vid){
        ...
        if($how=='verify'){
            if(!$vid){
                ...
                $vuid = M('member')->where(array('mobile'=>$adr,'verify_mobile'=>1))->getField('uid');
                if(!$vuid){
                    return array('status'=>1);
                }else{
                    return array('status' => 0, 'info' => "您的手机号已被占用请更换！");
