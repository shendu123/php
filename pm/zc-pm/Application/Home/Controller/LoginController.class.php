...
class LoginController extends CommonController {
    ...
    public function index(){
        if(IS_POST){
            ...
            $info = $member->field(array('uid','account','pwd','status','alerttype','business_id','verify_mobile','mobile', 'aid'))->where($where)->find();
            if(empty($info)){
    	...
    	if(IS_POST){
            ...
            switch (I('post.registerType')) {
                case 'account':
                    ...
                    if($member->where(array('mobile'=>I('post.mobile')))->count()!=0){
                        echojson(array('status' => 0, 'info' => I('post.mobile').'手机号已注册，如果忘记密码，请找回'));
                        exit;
                    }
                    ...
                    break;
                case 'email':
                    echojson(array('status' => 0, 'info' => '未开通的注册方式，请选择手机注册'));
                    exit;
                ...
                case 'mobile':
                    ...
                    if($member->where(array('account'=>I('post.mobile')))->count()!=0){
                        echojson(array('status' => 0, 'info' => I('post.account').'账号已存在，换一个吧'));
                        exit;
                    }
            ...
            $_SESSION['business_id'] = I('post.business_id');

            if($_COOKIE['tuijian_aid']){
                $data['aid'] = $_COOKIE['tuijian_aid'];
            }
            ...
            $mf = M('feedback');
            $mbcof=C('Member');
            $mbcof['register'] = array ('mobile' => 'on');
            ...
            if(!empty($link)){
                $bb = new Util();
                $hh = $bb->decrypt($link,$bb->salt);
                $hh = explode('/', $hh);
                setcookie("tuijian_aid", $hh[1], -3600, "/");
                setcookie("tuijian_buid", $hh[0], -3600, "/");

                $this->busi = $busi->where('business_id = '.$hh[0])->select();
                if(!$this->busi){
                    $this->busi = $busi->select();
                }
            }else{
                if($_COOKIE['tuijian_buid']){
                    $this->busi = $busi->where('business_id = '.$_COOKIE['tuijian_buid'])->select();
                    if(!$this->busi){
                        $this->busi = $busi->select();
                    }
                }else{
    ...
    public function sendCode(){
        ...
        if(I('post.checktp')=='email'){
            ...
            if($verifyME['status']){
                ...
                $noteStatus = sendNote($checkadr,$rc,I('post.type'));
