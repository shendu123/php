...
class PaymentController extends CommonController {
    public function index(){
    	...
    	// if($_SESSION['mobile']=='13067216009'){
    	// 	var_dump($sub_data);
    	// }
    	...
    	switch ($sub_data["payType"]) {
    		case 'recharge':
    			...
    			if ($sub_data['purpose']=='pledge') {
	    			...
	    			$adata = array(
	                    ...
	                    'update_time' => time(),
	                    'title'=>'充值余额',
	                    'return_url'=> U('Home/Member/pledge','','',true),
	                    // 'return_url'=> U('Home/Payment/webhookNew','','',true),
	                    'show_url'=> 'http://www.fjsxpmh.com/Home/Payment/webhookNew?bid='.$_SESSION['business_id']
	                );
	    			$adata['business_id'] = $_SESSION['business_id'];
	    			// var_dump($adata);exit();
	                $check = M("Payorder")->add($adata);
	                // var_dump(M("Payorder")->getDbError());
    		...
    		case 'auction':
	            ...
	            // 设置发货地址
	            $region = M('region');
	            $address = M('deliver_address')->where(array('adid'=>$sub_data['adid'],'uid'=>$this->cUid))->find();
	            if($address){
	                $address['province'] = $region->where(array('region_id'=>$address['province']))->getField('region_name');
	                $address['city'] = $region->where(array('region_id'=>$address['city']))->getField('region_name');
	                $address['area'] = $region->where(array('region_id'=>$address['area']))->getField('region_name');
	            ...
	            // 设置发货地址】
	            /*需要支付金额*/
	            $total = $order_info['price']+$order_info['freight'];
	            // 读取拍品订单
	            ...
	            if($bidinfo['sid']!=0){
	                ...
	                // 消息内容
	                $sms_con = '<a href="'.U('Special/speul',array('sid'=>$special['sid'],'aptitude'=>1)).'">【'.$special['sname'].'】</a>'.'专场内拍品【'.'<a href="'.U('Auction/details',array('pid'=>$bidinfo['pid'],'aptitude'=>1)).'">'.$bidinfo['pname'].'】</a>';
	                $special = M('special_auction')->where(array('sid'=>$bidinfo['sid']))->find();
	                if($special['special_pledge_type']==0){
	                    $greezwhere = array('g-u'=>'s-u','uid'=>$order_info['uid'],'gid'=>$bidinfo['sid'],'status'=>0);
	                }else{
	                   $greezwhere = array('g-u'=>'p-u','uid'=>$order_info['uid'],'gid'=>$order_info['gid'],'status'=>0); 
	                }
	            }else{
	            	// 消息内容
	                $sms_con = '<a href="'.U('Auction/details',array('pid'=>$bidinfo['pid'],'aptitude'=>1)).'">'.$bidinfo['pname'].'】</a>';
	                $greezwhere = array('g-u'=>'p-u','uid'=>$order_info['uid'],'gid'=>$order_info['gid'],'status'=>0);
	            }
	            // 如果勾选余额支付
	            if(in_array(0, $ptp)){
	                ...
	                $uLimit = $member->where(array('uid'=>$order_info['uid']))->field($ufield)->find();
	                // 可用保证金
	                $uLimit['usable'] = $uLimit['wallet_pledge']-$uLimit['wallet_pledge_freeze'];
	                $uLimit['usable'] = $uLimit['usable']>0?sprintf("%.2f", $uLimit['usable']):0;
	                // 用户可用余额】
	                $remaining += $sub_data['yuemn'];
	                // 可用余额小于支付用余额无法支付
	                
	                if($sub_data['yuemn']>$uLimit['usable']){
	                    $this->error('您输入的余额大于你的可用余额。请更正，否则无法进行支付！',U('Member/payment_order',array('order_no'=>$sub_data['order_no'])),5);
	                }else{
	                	$yuemn = $sub_data['yuemn'];
	                }
	            }else{
	                $yuemn = 0;
	            }
	            ...
	            if($sub_data['bpy']){
	                ...
	                // 获取支付拍品保证金金额
	                $pledge = $goods_user->where($greezwhere)->getField('pledge');
	                // 保证金足以支付订单
	                if ($pledge>=$total) {
	// -------------------------------------------全额保证金抵货款------------------------------------------------------
	                	// 全部用保证金抵货款支付【
		                    if($member->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge_freeze',$total)){
		                    	// 扣除余额
		                    	if($member->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge',$total)){
		                    		// 减去缴纳的保证金
		                    		if($goods_user->where($greezwhere)->setField('pledge',$pledge-$total)){
		                    			if (!D('Payment')->pledgepaySend($bidinfo,$total,$sub_data['order_no'],$order_info['uid'])) {
				                    		$this->error('写入余额记录失败，请联系管理员！',$adata["show_url"]);
				                    	}else{
		                    		...
		                    		$this->error('扣除保证金失败,请联系管理员！',$adata["show_url"]);
		                    	}
		                    }else{
		                    	$this->error('扣除保证金失败！请联系管理员！',$adata["show_url"]);
		                    }
	...
	// -------------------------------------------需要在线支付------------------------------------------------------
	            if($remaining<$total){
	                // 如果需要在线再付且用户没有勾选在线支付提示用户勾选在线支付
	                if(!in_array(1, $ptp)){
	                    $this->error('您所选余额不足以支付该订单。请勾选在线支付，否则无法进行支付！',U('Member/payment_order',array('order_no'=>$sub_data['order_no'])));
	                }else{
						$bill_no = createNo('zfp');
						// 计算需要在线支付的钱数
	                    $linpay = sprintf("%.2f",$total-$remaining); 
		    			// 生成本地支付订单保存【
		    			$adata = array(
		                    'bill_no' => $bill_no, 
		                    ...
		                    // 'show_url' => U('Auction/details',array('pid'=>$bidinfo['pid'],'',true)),
		                    'show_url'=> 'http://www.fjsxpmh.com/Home/Payment/webhookNew?bid='.$_SESSION['business_id'],
		                );
		                $piidd = $bidinfo['pid'];
		                $adata['business_id'] = $_SESSION['business_id'];
	...
	//----------------------------------------全部余额支付------------------------------------------------------------
	                ...
	                if(in_array(0, $ptp)){
	                    // 扣除余额
	                    if($member->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge',$sub_data['yuemn'])){
	                    	if (!D('Payment')->yuepaySend($bidinfo,$sub_data['yuemn'],$sub_data['order_no'],$order_info['uid'])) {
	                    		$this->error('写入余额记录失败，请联系管理员！',$adata["show_url"]);
	                    	}
	                    }else{
	                    	$this->error('扣除余额失败，请联系管理员！',$adata["show_url"]);
	                    }
	                }
	                // 保证金全额抵货款-----------------------------------------
	                if($sub_data['bpy']){
	                    if($pledge>0){
		                    // 扣除保证金
		                    if($member->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge_freeze',$pledge)){
		                    	// 扣除保证金
		                    	if($member->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge',$pledge)){
		                    		if (!D('Payment')->pledgepaySend($bidinfo,$pledge,$sub_data['order_no'],$order_info['uid'])) {
			                    		$this->error('写入余额记录失败，请联系管理员！',$adata["show_url"]);
			                    	}else{
			                    		$goods_user->where($greezwhere)->setField('pledge',0);
			                    	}
		                    	}else{
		                    		$this->error('扣除保证金失败,请联系管理员！',$adata["show_url"]);
		                    	}
            ...
            case 'crowd':
                ...
                $ptp = I('post.ptp');
                // 读取订单信息
                $order_info = D('CrowdOrder')->where(array('crd_no'=>$sub_data['order_no']))->find();
                if(empty($order_info) || $order_info['status'] != 0){
                    $this->error('订单已失效或不存在的订单号！', U("Home/Member/mycrowd",array('st'=>0)));
                }
                $remaining = $total = $yuemn = 0;
                // 设置发货地址
                $region = M('region');
                $address = M('deliver_address')->where(array('adid'=>$sub_data['adid'],'uid'=>$this->cUid))->find();
                if($address){
                    ...
                    $address['city'] = $region->where(array('region_id'=>$address['city']))->getField('region_name');
                    $address['area'] = $region->where(array('region_id'=>$address['area']))->getField('region_name');
                    D('CrowdOrder')->updateAddress($sub_data['order_no'], $address);
                }else{
                    $this->error('地址被删除，或不可用的地址！',U('Member/deliver_address'),5);
                }
                // 写入卖家留言
                D('CrowdOrder')->where(array('crd_no'=>$sub_data['order_no']))->setField('buyer_msg',$sub_data['buyer_msg']);
                // 设置发货地址】
                /*需要支付金额*/
                ...
                if(in_array(0, $ptp)){
                    // 用户可用余额【
                    $uLimit = D('Member')->where(array('uid'=>$order_info['uid']))->field(array('wallet_pledge','wallet_pledge_freeze'))->find();
                    // 可用保证金
                    $uLimit['usable'] = $uLimit['wallet_pledge']-$uLimit['wallet_pledge_freeze'];
                    $uLimit['usable'] = $uLimit['usable']>0?sprintf("%.2f", $uLimit['usable']):0;
                    // 用户可用余额】
                    ...
                    if($sub_data['yuemn'] > $uLimit['usable']){
                        $this->error('您输入的余额大于你的可用余额。请更正，否则无法进行支付！',U('Member/crd_payment_order',array('crd_no'=>$sub_data['order_no'])),5);
                    }else{
                        $yuemn = $sub_data['yuemn'];
                    }
                }
                $item = D('CrowdItems')->details($order_info['ciid']);
                // 计算是否需要在线支付
                // -------------------------------------------需要在线支付------------------------------------------------------
                if($remaining < $total){
                    // 如果需要在线再付且用户没有勾选在线支付提示用户勾选在线支付
                    if(!in_array(1, $ptp)){
                        $this->error('您所选余额不足以支付该订单。请勾选在线支付，否则无法进行支付！',U('Member/crd_payment_order',array('crd_no'=>$sub_data['order_no'])));
                    }else{
                        $bill_no = createNo('zfc');
                        // 计算需要在线支付的钱数
                        $linpay = sprintf("%.2f",$total - $remaining);
                        // 生成本地支付订单保存【
                        $adata = array(
                            ...
                            'update_time' => time(),
                            'title' => '支付申购订单',
                            'return_url'=>U('Member/mycrowd',array('st'=>1),'',true),
                            'show_url'=> 'http://www.fjsxpmh.com/Home/Payment/webhookNew?bid='.$_SESSION['business_id']
                            // 'show_url' => U('Auction/details',array('pid'=>$bidinfo['pid'],'',true))
                        );
                        $piidd = $sub_data['order_no'];
                        $adata['business_id'] = $_SESSION['business_id'];
                        $check = D("Payorder")->add($adata);
                        // var_dump(D("Payorder")->getDbError());
                        // var_dump($piidd);
                        $online = true;
                        // 生成本地支付订单保存】
                    }
        //             if($_SESSION['mobile']=='13067216009'){
                    ...
                    if(in_array(0, $ptp)){
                        // 扣除余额
                        if(D('Member')->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge', $sub_data['yuemn'])){
                        	error_log("setDec success", 3, '/tmp/yuezhifu.log');
                            if (!D('Payment')->yuepaySend($item,$sub_data['yuemn'],$sub_data['order_no'],$order_info['uid'])) {
                                $this->error('写入余额记录失败，请联系管理员！',$adata["show_url"]);
                            }
                        }else{
                            $this->error('扣除余额失败，请联系管理员！',$adata["show_url"]);
                    ...
                    if(D('CrowdOrder')->where(array('crd_no'=>$sub_data['order_no']))->setField(array('status'=>1,'time1'=>time()))){

                    	// 减少库存增加购买次数
                    	$item = D('CrowdItems')->where(array('ciid' => $order_info['ciid']))->find();
				        ...
				        if($item['is_limit_buy'] == 'yes' && $item['limit_stock'] > 0) {
				            if(D('CrowdItems')->where('ciid='.(int)$item['ciid'].' AND limit_stock > 0')->setDec('limit_stock', 1)) {
					            ...
					            $Model->rollback();
					            $this->error('库存扣减失败，请联系管理员或刷新页面重试！',$adata["show_url"]);
					        }
				        ...
				        if ($commit) {
				            ...
				            S('crowd_detail_'.$item['crowd_id'], null);
				            S('crowd_items_'.$item['crowd_id'], null);

	                        // 发送订单状态提醒
	                        sendOrderRemind($sub_data['order_no']);
	                        $this->success('支付成功',U('Home/Member/mycrowd',array('st'=>1)));
						} else {
							$Model->rollback();
							$this->error('支付失败，请联系管理员或刷新页面重试！',$adata["show_url"]);
						}
    	...
    	// 生成支付订单成功【
	    	...
	    	// 余额支付】
	    		...
	    		if($data["channel"]!="WX_NATIVE"){
		    		// 在线支付【
		    		header('Location: http://www.fjsxpmh.com/Home/Payment/online?bill='.$adata['bill_no'].'&pid='.$piidd);
		    		// header('Location: http://paimai/index.php/Home/Payment/online?bill='.$adata['bill_no'].'&pid='.$piidd);
		    		// $this->redirect('http://money.hddzsp.com/Home/Payment/online', array('bill' => $adata['bill_no']), 0, '页面跳转中...');
	    		}else{
	    			// if($_SESSION['mobile']=='13067216009'){
			    	// 	var_dump($adata);exit();
			    	// }
	    			header('Location: http://www.fjsxpmh.com/Member/payment?bill='.$adata['bill_no'].'&mon='.$sub_data["money"]);
	    		}
    ...
    public function online(){
        ...
        $da = M('payorder')->where(array('bill_no'=>$bill ))->find();

        if ($da['purpose']=='pledge') {
			$da["title"] = '充值余额';
		}

		if ($da['purpose']=='auction') {
			$da["title"] = '支付拍品订单';

			// $this->details($_GET['pid']);
		}
        ...
        //汇潮支付
	    $hh = new Huichaopay();
	    $result = $hh->payByHuichao($da['bill_no'],$da['money'],$da['title'],$da['title'],$da['return_url'],$da['show_url']);
	    echo $result;
    ...
    //原来的支付接口，改回去只要把online 互换就好了
    public function onlineOld(){
    	...
    	try {
		    switch ($data['channel']){
		        case 'BC_ALI_QRCODE':
		            ...
		            //break;
		        case 'ALI_OFFLINE_QRCODE':
		            $result = $result =  \beecloud\rest\api::offline_bill($data);
		            break;
		        default:
    ...
    Private function _jsapi ($data) {
		...
		if (!isset($_GET['code'])){
		    //触发微信返回code码
		    $url = $jsApi->createOauthUrlForCode(U('Home/Payment/online',array('channel'=>'WX_JSAPI','bill'=>$data["bill_no"]),'',true));
		    Header("Location: ".$url);
		    exit();
		} else {
		    //获取code码，以获取openid
 ...
 // 	$mvv['url'] = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.C('Weixin.appid').'&redirect_uri='.urlencode(substr(C('WEB_ROOT'), 0, -1).U('Home/Wlogin/index')).'&response_type=code&scope=snsapi_base&state='.$mvv['mark'].'#wechat_redirect';
 ...
 // }
    ...
    public function isorder(){
    	$bill = I('get.bill');
    	$status = M('payorder')->where(array('bill_no'=>$bill))->getField('status');
    	echo $status;
    }
    ...
    public function webhookNew(){
		...
		// $dp = array(
		// 	"MerNo"=> "21750" ,
		// 	"BillNo"=>  "zfc149481914420724" ,
		// 	"Amount"=> "90.00" ,
		// 	"Succeed"=> "88" ,
		// 	"OrderNo"=>  "0012199191" ,
		// 	"Result"=> "SUCCESS" ,
		// 	"SignInfo"=>  "4E4A2AFEE1229825F5BB033E140E5A24"
		// );
    	$type = I('get.type');
    	if($type=="wechat"){
    		$bill_no = $dp['BillNo'];
			...
			$transaction_fee =$dp['Amount'];
			$thisOrder = M('payorder')->where(array('bill_no'=>$bill_no))->find();
			$chongzhifangshi  = "微信支付";
			S('testmsga',$thisOrder);
    	}else{
    		$chongzhifangshi  = "汇潮支付";
			...
			S('testmsga',$thisOrder);
			if($thisOrder['money'] != $transaction_fee){
				// 支付订单金额不匹配
				echo "支付订单金额不匹配";
				exit();
			}
			error_log("start:{$bill_no}  {$thisOrder['purpose']}\r\n", 3, __DIR__.'/Payment_webhooknew.log');
	    ...
	    if($result=="SUCCESS"){
	    	if($thisOrder['purpose']=='pledge'){
	    		...
	    		if($status!=1){
		    		...
		    		$data = array(
		    			...
		    			'annotation'=>'【'.$chongzhifangshi.'】在线充值'.$thisOrder['money'].'元，充值成功！',
		    			'income'=>$thisOrder['money']
		    		);

		    		if(I('get.bid')!=null){
		    			$data["business_id"] = I('get.bid');
		    		}
		    		...
		    		//$staz = $member->where(array('uid'=>$thisOrder['uid']))->setInc('point',$thisOrder['money']/10);
		    	// 提醒通知充值成功【
	                ...
	                $mail_pledge['msg'] = '您好：<br/><p>'.$data['annotation'].'</p><p>您可以<a target="_blank" href="'.U('Home/Member/pledge','','html',true).'">查看账户记录</a></p>';
	    	...
	    	if($thisOrder['purpose']=='auction'){
	    		$goods_order = M('goods_order');
	    		$status = $goods_order->where(array('order_no'=>$thisOrder['order_no']))->getField('status');
	    		if($status==0){
	    			// 计算发货过期时间
	    			if(C('Order.losetime2')==0||C('Order.losetime2')==''){
		                $deftime2 = 0;
		    		...
		    		// 读取商品信息
		    		$bidinfo = M('auction')->where(array('pid'=>$order_info['gid']))->field('pid,pname')->find();
		    		// 设置支付订单为已支付
		    		M('payorder')->where(array('bill_no'=>$thisOrder['bill_no']))->setField('status',1);
		    		// 当设置有余额支付时候操作
		    		if($thisOrder['yuemn']>0){
		    			// 扣除余额
	                    if(M('member')->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge',$thisOrder['yuemn'])){
	                    	if (!D('Payment')->yuepaySend($bidinfo,$thisOrder['yuemn'],$thisOrder['order_no'],$thisOrder['uid'])) {
	                    		$this->error('写入余额记录失败，请联系管理员！');
	                    	}
	                    }else{
	                    	echo'扣除余额失败，请联系管理员！';
	                    }
		    		...
		    		if($thisOrder['pledge']>0){
		    			// 扣除保证金
	                    if(M('member')->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge',$thisOrder['pledge'])){
	                    	if(M('member')->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge_freeze',$thisOrder['pledge'])){
	                    		if (!D('Payment')->pledgepaySend($bidinfo,$thisOrder['pledge'],$thisOrder['order_no'],$thisOrder['uid'])) {
		                    		echo '写入余额记录失败';
		                    	}
	                    	}else{
	                    		echo '解冻保证金失败！';
            ...
            if($thisOrder['purpose']=='crowd'){
                $status = D('CrowdOrder')->where(array('crd_no'=>$thisOrder['order_no']))->getField('status');
				error_log("{$bill_no} status:{$status}\r\n", 3, __DIR__.'/Payment_webhooknew.log');
                if($status == 0){
                    // 设置已支付
                    D('CrowdOrder')->where(array('crd_no'=>$thisOrder['order_no']))->setField(array('status'=>1,'time1'=>time()));
                    // 读取订单信息
                    $order_info = D('CrowdOrder')->where(array('crd_no'=>$thisOrder['order_no']))->find();
                    // 读取申购子项信息
                    $item = D('CrowdItems')->details($order_info['ciid']);
                    ...
                    if($thisOrder['yuemn']>0){
                        // 扣除余额
                        if(M('member')->where(array('uid'=>$order_info['uid']))->setDec('wallet_pledge',$thisOrder['yuemn'])){
                            if (!D('Payment')->yuepaySend($item, $thisOrder['yuemn'], $thisOrder['order_no'], $thisOrder['uid'])) {
                            	$pay_success = 0;
                                $this->error('写入余额记录失败，请联系管理员！');
                            }
					...
					error_log("{$bill_no} pay_success:{$pay_success}\r\n", 3, __DIR__.'/Payment_webhooknew.log');
                    if ($pay_success) {
	                    $commit = $item['is_limit_buy'] == 'yes' ? false :true;
				        if($item['is_limit_buy'] == 'yes' && $item['limit_stock'] > 0) {
				            if(D('CrowdItems')->where('ciid='.(int)$item['ciid'].' AND limit_stock > 0')->setDec('limit_stock', 1)) {
					            ...
					            $this->error('库存减少失败，请联系管理员或刷新页面重试！',$adata["show_url"]);
					        }
					    }
						error_log("{$bill_no} commit:{$commit}\r\n", 3, __DIR__.'/Payment_webhooknew.log');
					    if ($commit) {
				            D('CrowdItems')->where(array('ciid' => $item['ciid']))->setInc('support_count', 1);
				            D('CrowdItems')->where(array('ciid' => $item['ciid']))->setInc('support_money', $item['price']);
				            D('Crowd')->where(array('crowd_id' => $item['crowd_id']))->setInc('support_count', 1);
    ...
    public function webhook(){
		...
		$thisOrder = M('payorder')->where(array('bill_no'=>$bill_no))->find();
		S('testmsga',$thisOrder);
		if($thisOrder['money']*100 != $transaction_fee){
			// 支付订单金额不匹配
			exit();
		}
		// 此处需要验证购买的产品与订单金额是否匹配:
