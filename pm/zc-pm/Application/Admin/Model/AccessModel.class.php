...
class AccessModel extends Model {
    ...
    public function roleList() {
        $M = M("Role");
        $list = $M->where(array('business_id'=>$_SESSION['business_id']))->select();
        foreach ($list as $k => $v) {
    ...
    public function adminList() {
        ...
        $list = M("Admin")->where(array('business_id'=>$_SESSION['business_id']))->select();
        foreach ($list as $k => $v) {
            $list[$k]['statusTxt'] = $v['status'] == 1 ? "启用" : "禁用";
    ...
    public function addAdmin() {
        ...
        $datas['business_id'] = $_SESSION['business_id'];
        if ($M->add($datas)) {
            M("RoleUser")->add(array('user_id' => $M->getLastInsID(), 'role_id' => (int) $_POST['role_id']));
            if (C("SYSTEM_EMAIL")) {
                $body = "你的账号已开通，登录地址：" . U('Public/index','','html',true) . "<br/>登录账号是：" . $datas["email"] . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;登录密码是：" . $_POST['pwd'];
    ...
    public function addRole() {
        $M = M("Role");
        $_POST['business_id'] = $_SESSION['business_id'];
        if ($M->add($_POST)) {
    ...
    public function changeRole() {
        ...
        foreach ($data as $k => $v) {
            ...
            $datas[$k]['level'] = $tem[1];
            $datas[$k]['pid'] = $tem[2];
            $datas[$k]['business_id'] = $_SESSION['business_id'];
        }
        if ($M->addAll($datas)) {
