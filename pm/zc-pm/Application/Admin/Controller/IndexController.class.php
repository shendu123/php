...
class IndexController extends CommonController {
    public function index() {
        ...
        $sw['business_id']=$_SESSION['business_id'];
        $this->takeAll = $take->where($sw)->count();
        $this->takeUn = $take->where('status=0 and business_id = '.$_SESSION['business_id'] )->count();
        // 过期拍卖订单
        $lbid = array(
            'business_id'=>array('eq',$_SESSION['business_id']),
        ...
        $Emc  = memcache_connect('127.0.0.1', '11211');
        $py_time = memcache_get($Emc,'orderCheck_py_isrun_time');

        $mm = intval(time())-intval($py_time);
        if($mm<600){
            $status_by = memcache_get($Emc,'orderCheck_py_isrun');
            switch ($status_by) {
                case '999':
                    $this->kk = 8;
                    $this->ckor_py_state = "py正常运行";
                    $this->ckor_py_tip = "接口正常 脚本正常";
                    break;
                case '998':
                    $tip = memcache_get($Emc,'orderCheck_py_tip');
                    $this->ckor_py_state = "py正常运行";
                    $this->ckor_py_tip = "接口返回错误:".$tip." 脚本运行正常";
                    break;
                case '997':
                    $this->ckor_py_state = "正常运行";
                    $this->ckor_py_tip = "脚本请求接口服务器错误 脚本正常";
                    break;
                default:
                    $this->kk = 9;
                    $this->ckor_py_state = "未运行脚本";
                    $this->ckor_py_tip = "请手动开启";
                    break;
            }
        }else{
            $this->kk = 9;
            $this->ckor_py_state = "未启动，或出现错误";
            $this->ckor_py_tip = "请手动开启，上次运行时间离现在已过".$mm."秒";
        }
...
//            'MYSQL版本' => mysql_get_server_info(),
    ...
    public function statistics(){
        ...
        if(I('get.')){
            ...
            if(I('get.account')!=''){
                $user = $member->where(array('account'=>I('get.account'),'business_id'=>$_SESSION['business_id']))->field('uid,nickname,account')->find();
                if ($user) {
                    $where['uid'] = $user['uid'];
                    $uwhere['uid'] = $user['uid'];
            ...
            if($take->save($sdata)){
                ...
                if($post['status']==1){
                    if($member->where($w)->setDec('wallet_pledge_freeze',$post['money'])&&$member->where($w)->setDec('wallet_pledge',$post['money'])){
                        ...
                        $pledge_data['business_id'] = $_SESSION['business_id'];
...
// var_dump($_SESSION);exit();
            ...
            $budi = $_SESSION['business_id'];
            $aid = $_SESSION['my_info']['aid'];

            $mm= M('member_expand_link');
            $mm1 = $mm->where(array('aid'=>$aid))->select();
// var_dump($budi);exit();
// var_dump($aid);exit();
            if(!$mm1||$mm1==null||$re=='9'){
                
                $bb = new Util();
                //business_id/推薦人id
                $rr = $bb->encrypt($budi.'/'.$aid.'/uijhygy6',$bb->salt);
                $url = C('WEB_ROOT')."Login/register/link/".$rr;

                $ee = $bb->createQRcode($url);
                $this->qrurl = $ee;
                $this->url = $url;
                ...
                $da['qrurl'] = $ee ;
                if($re=='9'){
                    $mm->where(array('aid'=>$aid))->save($da);
                }else{
                    $mm->add($da);
                }
                
            }else{
                $this->qrurl = $mm1[0]['qrurl'];
                $this->url = $mm1[0]['url'];
            }
// $this->qrurl = 'ff';
// var_dump($this->qrurl);exit();
           $this->display();
        }
    }
...
/**
    ...
    public function system_log() {
        ...
        // 用户类型
        if(I('get.user_type')!=''){
            $where['user_type']=I('get.user_type');
        }
        // 操作状态
        if(I('get.status')!=''){
        ...
        $data = $M->order('id desc')->limit($pConf['first'], $pConf['list'])->where($where)->select();           
        $this->list = $data;
        $this->page = $pConf['show'];
        $this->keys=I('get.');
        $this->display(); 
