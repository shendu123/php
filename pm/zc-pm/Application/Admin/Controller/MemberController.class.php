...
class MemberController extends CommonController {
    ...
    public function index() {
        ...
        $count = $M->where(array('business_id' =>$_SESSION['business_id'] ))->count();
        $pConf = page($count,C('PAGE_SIZE')); // 分页
        $list=$M->order('uid desc')->where(array('business_id' =>$_SESSION['business_id'] ))->limit($pConf['first'], $pConf['list'])->select();
        // $list6=$M->order('uid desc')->limit($pConf['first'], $pConf['list'])->select();
        // var_dump($list);
        // var_dump($list6);exit();
