...
class AuctionController extends CommonController {
    ...
    public function audit() {
        ...
        //增加企业id
        $ws['Auction_audit.business_id'] = array('eq', intval($_SESSION['business_id']));
        $ws['Auction_audit.audit_status'] = array('eq', '0');
        $count = D("Auction_audit")->where($ws)->count();

        $pConf = page($count,C('PAGE_SIZE'));
        $this->page = $pConf['show'];

        $this->list = D("Auction_audit")->listAuction($pConf['first'], $pConf['list'], $ws, $od);
        $this->saytyp = '待审核拍卖';

        $this->display();
    }

    public function auditPass() { // AJMstr【2017-3-21】: TODO optimize transaction
        if (IS_POST) {
            $M = M();
            $M->startTrans();
            $auction = $M->table('on_auction_audit')->where(array('pid' => I('post.pid')))->find();

            if($commit = $M->table('on_auction_audit')->where(array('pid' => I('post.pid')))->setField('audit_status', '1')) {
                if($commit = M('CrowdOrder')->where(array('crd_no' => $auction['crd_no']))->setField('auction_status', '3')) {
                    $audit_pid = $auction['pid'];
                    unset($auction['pid'], $auction['pid'], $auction['audit_status']);
                    $commit = $pid = M('Auction')->add($auction);
                    if($pid) {
                        M('seller_pledge')->where(array('pid' => $audit_pid, 'from' => 'auction_audit'))->save(array('pid' => $pid, 'from' => 'auction'));
                    }
                }
            }
            if($commit) {
                $M->commit();
                echojson(array('status' => 1, 'msg'=>'审核通过！'));
            } else {
                $M->rollback();
                echojson(array('status' => 1, 'msg'=>'操作失败，可能是不存在该PID的记录'));
            }
        }else{
            E('页面不存在！');
        }
    ...
    public function auditReject() {
        if (IS_POST) {
            ...
            if($commit = $M->table('on_auction_audit')->where(array('pid' => I('post.pid')))->setField('audit_status', '2')) {
                unfreeze_seller_pledge($auction['sellerid'], I('post.pid'), 'audit_reject', 'auction_audit'); // 解冻卖家保证金
                $commit = M('CrowdOrder')->where(array('crd_no' => $auction['crd_no']))->setField('auction_status', '0');
            }

            if($commit) {
                $M->commit();
                echojson(array('status' => 1, 'msg'=>'成功拒绝！'));
            } else {
                $M->rollback();
                echojson(array('status' => 0, 'msg'=>'操作失败，可能是不存在该PID的记录'));
            }
        }else{
            E('页面不存在！');
        }
    ...
    public function index() {
        ...
        if($business_id!=null){
            $ws['bidType']['Auction.business_id'] = array('eq',intval($business_id));
        }
    ...
    public function clearHome(){
        echo "start";
        // echo delDirAndFile(WEB_CACHE_PATH . "Cache/Home/");
    }
// 撤拍操作
...
// 删除操作
    public function del(){
        if (IS_POST) {
            ...
            if ($auctionB->where($where)->delete()) {
                // 解冻卖家保证金
    ...
    // 拍卖会--------------------------------------------end
                ...
                // var_dump($where);exit();
                // echojson(array('status' => 1, 'msg'=>$auctionB->getDbError()));
                echojson(array('status' => 1, 'msg'=>$where));
            }
        }else{
            E('页面不存在！');
        }
...
//以下专场相关-----------------------------------------------------------------------------------------
    ...
    public function special(){
        ...
        $business_id = $_SESSION['business_id'];
        if($business_id!=null){
            $ws['bidType']['business_id'] = array('in',$business_id);
        }
