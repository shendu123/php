...
class WeixinController extends CommonController {
    ...
    public function autoreply(){
        if(IS_POST){
            ...
            if($id){
                $data['id']=$id;
                $res=M('weiautoreply')->save($data);
                $msg='修改';
            }else{
                $res=M('weiautoreply')->add($data);
                $msg='添加';
            }
            if($res!==false){
                echojson (array('status' => 1, 'info' => $msg.'成功！','url'=>U('Weixin/autoreply')));
            }else{
                echojson( array('status' => 0, 'info' => $msg.'失败！','url'=>U('Weixin/autoreply')));
            }
        }else{
            $this->info=M('weiautoreply')->find();
            $this->display();
        }
