...
class Rbac
{
    ...
    public static function AccessCheck($actionName = null, $controllerName = null, $moduleName = null)
    {
        ...
        if (self::checkAccess()) {
            //存在认证识别号，则进行进一步的访问决策
            if (!Cache::has(Config::get('rbac.admin_auth_key'))) {
                if (null === $moduleName) {
                ...
                if (2 == Config::get('rbac.user_auth_type')) {
                    ...
                    $userId = Cache::get(Config::get('rbac.user_auth_key'));
                    // 将权限缓存5分钟
...
//                    }//print_r(Cache::get($cacheKey));exit;
    ...
    public static function getAccessList($authId)
    {
        ...
        $sql = "SELECT node.id,node.name,node.pid,node.title FROM " .
            $table['role'] . " AS role," .
            $table['user'] . " AS user," .
            ...
            "AND node.status=1 ";
        $apps = Db::query($sql);
        $arr=['id'=>1,'name'=>'Admin','pid'=>0];//一级节点
        array_unshift($apps,$arr);
        //转化为树
        $tree = list_to_tree($apps);//print_r($apps);exit;
        //递归生成权限树
    ...
    private static function treeToMultiArray($tree, $key = "name", $key_default = "id", $key_child = "_child")
    {
        ...
        if (is_array($tree)) {
            foreach ($tree as $v) {
                ...
                $nodes = explode("/", str_replace(".", "/", $v[$key]));
