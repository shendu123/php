...
class Controller
{
    ...
    {   
        ...
        // 用户ID
        defined('UID') or define('UID', Cache::get(Config::get('rbac.user_auth_key')));
        // 是否是管理员
        defined('ADMIN') or define('ADMIN', true === Cache::get(Config::get('rbac.admin_auth_key')));
        // 检查认证识别号
        if (!UID) {
           header("location:http://".$_SERVER['HTTP_HOST'].'/admin/login/checklogin');
            //$this->notLogin();
        } else {
            $this->auth();
        }
    ...
    protected function auth()
    {
        ...
        if (
            ...
            !in_array($this->request->module(), explode(',', Config::get('rbac.not_auth_module')))
              ...
              if(\Rbac::AccessCheck()){
                // print_r(\Rbac::AccessCheck());exit; 
                  return \Rbac::AccessCheck();
              }
            
//            if (!\Rbac::AccessCheck()) {
//                echo json_encode(['message'=>'没有权限']);exit;
//                //return json(['message'=>'没有权限'],403);exit;
