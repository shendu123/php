...
class AdminRole
{
    ...
    public function getAccessTree($role_id)
    {
        ...
        if (!ADMIN) {
            $access_node = Db::name("RoleNode")->alias("access")->join("__ADMIN_ROLE_USER__ role_user", "role_user.role_id = access.role_id")->field("access.node_id")->where("role_user.user_id", UID)->select();
